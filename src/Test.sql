-- 1. Owner_лердин аттарынын арасынан эн коп символ камтыган ow7ner_ди жана анын уйун(House) чыгар.

        select o.first_name AS owner_name, h.house_type from owners o join houses h
            ON o.id = h.owner_id order by length(o.first_name) desc limit 1;

-- 2. Уйлордун баалары 1500, 2000 дин аралыгында бар болсо true чыгар, жок болсо false чыгар.
    select * from houses where price between 1500 and 2000;
...
-- 5. Клиенттердин 10-катарынан баштап 1999-жылдан кийин туулган 15 клиентти чыгар.
    select * from customers
        offset 10 limit '1999-01-01';


-- 6. Рейтинги боюнча уйлорду сорттоп, уйлордун тайптарын, рейтингин жана уйдун ээлерин чыгар. (asc and desc)
        select h.house_type, h.rating, h.owner_id * from houses h
        join owners o on o.id = h.owner_id order by asc;

-- 7. Уйлордун арасынан квартиралардын (apartment) санын жана алардын баасынын суммасын чыгар.
    select count(*) as count_houses, sum(price) as total_price from houses where house_tupe =  'apartment';



-- 8. Агентсволардын арасынан аты ‘My_House’ болгон агентсвоны, агентсвонын адресин жана анын бардык уйлорун,
--      уйлордун    адрессин чыгар.
        select a.name, a.address_id, * from agencies a

        select a.name as agency_name,
            a.address_id as agency_address,
            h.address_id as house_address
                from agencies a join houses h
                     join rent_info ri on a.id = ri.agency_id
            on a.id = h.agencies_id
...
create type House_tupe as Enum ('House', 'Apartment');
...
alter table houses
    rename numeric_price to price;
ALTER TABLE houses
    ADD COLUMN house_type VARCHAR;
